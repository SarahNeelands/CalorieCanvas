...
export default function ProfileSetup3() {
  ...
  function onTargetWeightUnitChange(nextUnit) {
    ...
    if (targetWeight) {
      ...
      if (Number.isFinite(numericValue)) {
        const convertedValue = nextUnit === 'lb' ? lbFromKg(numericValue) : kgFromLb(numericValue);
  ...
  async function onNext(e) {
    ...
    const twNum = tw ? (targetWeightUnit === 'lb' ? kgFromLb(parseFloat(tw)) : parseFloat(tw)) : null;
        ...
        <section className="ps-left">
          ...
          <form onSubmit={onNext} className="ps-form">
            <div className="ps-row">
              <div className="ps-col">
                ...
                <div className="ps-row" style={{ gridTemplateColumns: '1fr auto', gap: 12 }}>
                  {heightUnit === 'cm' ? (
                    <input className="ps-input" inputMode="decimal" value={heightCm} onChange={(e) => setHeightCm(cleanNum(e.target.value))} placeholder="cm" />
                  ) : (
                    <div className="ps-row" style={{ gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                      <input className="ps-input" inputMode="numeric" value={ft} onChange={(e) => setFt(cleanNum(e.target.value, false))} placeholder="ft" />
                      <input className="ps-input" inputMode="decimal" value={inch} onChange={(e) => setInch(cleanNum(e.target.value))} placeholder="in" />
                    </div>
                  )}
                  <select className="ps-input ps-select" value={heightUnit} onChange={(e) => onHeightUnitChange(e.target.value)}>
              ...
              <div className="ps-col">
                ...
                <div className="ps-row" style={{ gridTemplateColumns: '1fr auto', gap: 12 }}>
                  {weightUnit === 'kg' ? (
                    <input className="ps-input" inputMode="decimal" value={weightKg} onChange={(e) => setWeightKg(cleanNum(e.target.value))} placeholder="kg" />
                  ) : (
                    <input className="ps-input" inputMode="decimal" value={lb} onChange={(e) => setLb(cleanNum(e.target.value))} placeholder="lb" />
                  )}
                  <select className="ps-input ps-select" value={weightUnit} onChange={(e) => onWeightUnitChange(e.target.value)}>
            ...
            <div className="ps-col">
              ...
              <select id="activityLevel" className="ps-input ps-select" value={activityLevel} onChange={(e) => setActivityLevel(e.target.value || 'sedentary')}>
                {activityOptions.map((option) => (
            ...
            <div className="ps-row">
              <div className="ps-col">
                ...
                <div className="ps-row" style={{ gridTemplateColumns: '1fr auto', gap: 12 }}>
                  ...
                  />
                  <select className="ps-input ps-select" value={targetWeightUnit} onChange={(e) => onTargetWeightUnitChange(e.target.value)}>
                    <option value="kg">kg</option>
                    <option value="lb">lb</option>
                  </select>
                </div>
              ...
              <div className="ps-col">
                ...
                <input className="ps-input" inputMode="decimal" value={targetBf} onChange={(e) => setTargetBf(cleanNum(e.target.value))} placeholder="e.g., 22" />
              </div>
            </div>
            ...
            <div className="ps-actions">
              <a className="ps-back" href="/profile-setup-2">&larr; Back</a>
